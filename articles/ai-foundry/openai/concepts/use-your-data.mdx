...
<Frame>
    ![](/images/ai-foundry/openai/media/use-your-data/workflow-diagram.png)
</Frame>
...
To get started, [connect your data source](../use-your-data-quickstart) using [Azure AI Foundry portal](https://ai.azure.com/?cid=learnDocs) and start asking questions and chatting on your data.
...
There's an [upload limit](../quotas-limits), and there are some caveats about document structure and how it might affect the quality of responses from the model: 
...
<Frame>
    ![](/images/ai-foundry/openai/media/use-your-data/azure-databases-and-ai-search.png)
</Frame>
...
* [Vector search](/azure/search/vector-search-overview) using Ada [embedding](./understand-embeddings) models, available in [selected regions](models.md#embeddings-models) 
...
<Frame>
    ![](/images/ai-foundry/openai/media/use-your-data/index-data-mapping.png)
</Frame>
...
Mapping these fields correctly helps ensure the model has better response and citation quality. You can additionally configure it [in the API](../references/on-your-data) using the `fieldsMapping` parameter.   

### Search filter (API)

If you want to implement additional value-based criteria for query execution, you can set up a [search filter](/azure/search/search-filters) using the `filter` parameter in the [REST API](../references/azure-search).

import AiSearchIngestion from '/snippets/ai-foundry/openai/includes/ai-search-ingestion.mdx'

<AiSearchIngestion />

import Authentication from '/snippets/ai-foundry/openai/includes/on-your-data-authentication.mdx'
...
* A deployed [embedding model](../concepts/understand-embeddings)
...
<Frame>
    ![](/images/ai-foundry/openai/media/use-your-data/mongo-index-mapping.png)
</Frame>
...
1. [Add a data source](../quickstart) using [Azure AI Foundry portal](https://ai.azure.com/?cid=learnDocs).
1. Under **Select or add data source** select **Indexer schedule** and choose the refresh cadence you would like to apply.

    <Frame>
        ![](/images/ai-foundry/openai/media/use-your-data/indexer-schedule.png)
    </Frame>
...
1. Select **Indexers** from the left pane 
    ...
    <Frame>
        ![](/images/ai-foundry/openai/media/use-your-data/indexers-azure-portal.png)
    </Frame>
...
1. Perform the following steps on the two indexers that have your index name as a prefix.
    ...
    1. Update the schedule to the desired cadence from "Schedule" or specify a custom cadence from "Interval (minutes)"
        ...
        <Frame>
            ![](/images/ai-foundry/openai/media/use-your-data/indexer-schedule-azure-portal.png)
        </Frame>
...
Using [Azure AI Foundry portal](https://ai.azure.com/?cid=learnDocs), you can upload files from your machine to try Azure OpenAI On Your Data. You also have the option to create a new Azure Blob Storage account and Azure AI Search resource. The service then stores the files to an Azure storage container and performs ingestion from the container. You can use the [quickstart](../use-your-data-quickstart) article to learn how to use this data source option.

<Frame>
    ![](/images/ai-foundry/openai/media/quickstarts/add-your-data-source.png)
</Frame>
...
1. Set up [Elasticsearch](https://www.elastic.co/guide/en/elasticsearch/reference/current/setup.html) and get your connection information. 
    ...
    <Frame>
        ![](/images/ai-foundry/openai/media/use-your-data/connect-elasticsearch.png)
    </Frame>
...
Along with using Elasticsearch databases in [Azure AI Foundry portal](https://ai.azure.com/?cid=learnDocs), you can also use your Elasticsearch database using the [API](../references/elasticsearch). 
...
<Frame>
    ![](/images/ai-foundry/openai/media/use-your-data/mongo-db-network-access.png)
</Frame>
...
<Frame>
    ![](/images/ai-foundry/openai/media/use-your-data/mongo-db-atlas-connection.png)
</Frame>
...
<Frame>
    ![](/images/ai-foundry/openai/media/use-your-data/mongo-db-atlas-source-index.png)
</Frame>
...
<Frame>
    ![](/images/ai-foundry/openai/media/use-your-data/mongo-db-atlas-field-mapping.png)
</Frame>
...
Deploying to a standalone web app lets you and your users to interact with chat models through a graphical user interface. See [Use the Azure OpenAI web app](../how-to/use-web-app) for more information. 

---

## Configure access and networking for Azure OpenAI On Your Data

You can use Azure OpenAI On Your Data and protect data and resources with Microsoft Entra ID role-based access control, virtual networks, and private endpoints. You can also restrict the documents that can be used in responses for different users with Azure AI Search security filters. See [Azure OpenAI On Your Data access and network configuration](../how-to/on-your-data-configuration).
...
You can modify the following additional settings in the **Data parameters** section in [Azure AI Foundry portal](https://ai.azure.com/?cid=learnDocs) and [the API](../references/on-your-data). You don't need to reingest your data when you update these parameters. 
...
Once you select add your dataset, you can use the **System message** section in the [Azure AI Foundry portal](https://ai.azure.com/?cid=learnDocs), or the `role_information` [parameter in the API](../references/on-your-data).

<Frame>
    ![](/images/ai-foundry/openai/media/use-your-data/system-message.png)
</Frame>
...
* [Chain-of-thought prompting](prompt-engineering) has been shown to be effective in getting the model to produce desired outputs for complex questions/tasks.
...
Some Azure OpenAI models allow you to define [tools and tool_choice parameters](../how-to/function-calling) to enable function calling. You can set up function calling through [REST API](../reference.md#chat-completions) `/chat/completions`. If both `tools` and [data sources](../references/on-your-data.md#request-body) are in the request, the following policy is applied.
...
If the policy above doesn't meet your need, please consider other options, for example: [prompt flow](/azure/machine-learning/prompt-flow/overview-what-is-prompt-flow) or [Assistants API](../how-to/assistant).
...
## Next steps
* [Get started using your data with Azure OpenAI](../use-your-data-quickstart)

* [Azure OpenAI On Your Data network and access configuration](../how-to/on-your-data-configuration)

* [Introduction to prompt engineering](./prompt-engineering)
