...
Registration is required for access to `o3-mini`. For more information see, our [reasoning models guide](./reasoning).
...
<Note>
In the [Azure AI Foundry portal](https://ai.azure.com/?cid=learnDocs) the batch deployment types will appear as `Global-Batch` and `Data Zone Batch`. To learn more about Azure OpenAI deployment types, see our [deployment types guide](../how-to/deployment-types).
</Note>

<Frame>
    ![](/images/ai-foundry/openai/media/how-to/global-batch/global-batch.png)
</Frame>
...
import AzureAiFoundryPortal from '/snippets/ai-foundry/openai/includes/batch/batch-studio.mdx'

<AzureAiFoundryPortal />

</Tab>

<Tab title="Python">

import Python from '/snippets/ai-foundry/openai/includes/batch/batch-python.mdx'
...
import Rest from '/snippets/ai-foundry/openai/includes/batch/batch-rest.mdx'

<Rest />

</Tab>
</Tabs>
import Quota from '/snippets/ai-foundry/openai/includes/global-batch-limits.mdx'
...
| `input_no_permissions` | It's not possible to access the input blob. Please check [permissions](./role-based-access-control) and network access between the Azure OpenAI account and Azure Storage account.  |
...
* Learn more about Azure OpenAI [deployment types](./deployment-types)
