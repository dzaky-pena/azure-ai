---
title: "How to migrate to OpenAI Python v1.x in Azure AI Foundry Models"
...
- **`DALL-E3` is [fully supported with the latest 1.x release](../dall-e-quickstart).** `DALL-E2` can be used with 1.x by making the [following modifications to your code](#dall-e-fix).
...
Additional examples can be found in our [in-depth Chat Completion article](chatgpt).
...
Additional examples including how to handle semantic text search without `embeddings_utils.py` can be found in our [embeddings tutorial](../tutorials/embeddings).
...
For the full configuration steps that are required to make these code examples work, consult the [use your data quickstart](../use-your-data-quickstart).
