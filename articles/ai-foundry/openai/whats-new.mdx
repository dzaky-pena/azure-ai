...
- `codex-mini` and `o3-pro` are now available. To learn more, see the [getting started with reasoning models page](./how-to/reasoning)
...
Follow the [Video generation quickstart](/azure/ai-services/openai/video-generation-quickstart) to get started. For more information, see the [Video generation concepts](./concepts/video-generation) guide.

### Spotlighting for prompt shields

Spotlighting is a sub-feature of prompt shields that enhances protection against indirect (embedded document) attacks by tagging input documents with special formatting to indicate lower trust to the model. For more information, see the [Prompt shields filter](./concepts/content-filter-prompt-shields) documentation.


### Model router (preview)

Model router for Azure AI Foundry is a deployable AI chat model that automatically selects the best underlying chat model to respond to a given prompt. For more information on how model router works and its advantages and limitations, see the [Model router concepts guide](./concepts/model-router). To use model router with the Completions API, follow the [How-to guide](./concepts/model-router).
...
`o4-mini` and `o3` models are now available. These are the latest reasoning models from Azure OpenAI offering significantly enhanced reasoning, quality, and performance. For more information, see the [getting started with reasoning models page](./how-to/reasoning).
...
The [Responses API](./how-to/responses) is a new stateful API from Azure OpenAI. It brings together the best capabilities from the chat completions and assistants API in one unified experience. The Responses API also adds support for the new `computer-use-preview` model which powers the [Computer use](./how-to/computer-use) capability.
...
<Frame>
    ![](/images/ai-foundry/openai/media/computer-use-preview.gif)
</Frame>
...
For more information, see our [reasoning model guide](./how-to/reasoning). 

### gpt-4o mini audio released

The `gpt-4o-mini-audio-preview` (2024-12-17) model is the latest audio completions model. For more information, see the [audio generation quickstart](./audio-completions-quickstart).

The `gpt-4o-mini-realtime-preview` (2024-12-17) model is the latest real-time audio model. The real-time models use the same underlying GPT-4o audio model as the completions API, but is optimized for low-latency, real-time audio interactions. For more information, see the [real-time audio quickstart](./realtime-audio-quickstart).
...
`o3-mini` (2025-01-31) is the latest reasoning model, offering enhanced reasoning abilities. For more information, see our [reasoning model guide](./how-to/reasoning).
...
The `gpt-4o-audio-preview` model introduces the audio modality into the existing `/chat/completions` API. The audio model expands the potential for AI applications in text and voice-based interactions and audio analysis. Modalities supported in `gpt-4o-audio-preview` model include:  text, audio, and text + audio. For more information, see the [audio generation quickstart](./audio-completions-quickstart).

<Note>
The [Realtime API](./realtime-audio-quickstart) uses the same underlying GPT-4o audio model as the completions API, but is optimized for low-latency, real-time audio interactions.
</Note>
...
- Added support for [prompt caching](./how-to/prompt-caching) with the `gpt-4o-realtime-preview` model.
...
For more information, see the [GPT-4o real-time audio quickstart](realtime-audio-quickstart) and the [how-to guide](./how-to/realtime-audio).
...
To learn more about the advanced `o1` series models see, [getting started with o1 series reasoning models](./how-to/reasoning).
...
[Stored completions](./how-to/stored-completions) allow you to capture the conversation history from chat completions sessions to use as datasets for [evaluations](./how-to/evaluations) and [fine-tuning](./how-to/fine-tuning).

### GPT-4o 2024-11-20

`gpt-4o-2024-11-20` is now available for [global standard deployment](./how-to/deployment-types) in:
...
[Vision fine-tuning](./how-to/fine-tuning) allows you to add images to your JSONL training data. Just as you can send one or many image inputs to chat completions, you can include those same message types within your training data. Images can be provided either as URLs or as base64 encoded images.
...
For more information on [getting started with global batch deployments](./how-to/batch).
...
For more information, see the [GPT-4o real-time audio quickstart](realtime-audio-quickstart).

### Global batch support updates

Global batch now supports GPT-4o (2024-08-06). See the [global batch getting started guide](./how-to/batch) for more information.
...
For more information see our [reference documentation](./reference-preview)
...
- New blog post on [getting started with model evaluations](https://techcommunity.microsoft.com/t5/ai-azure-ai-services-blog/how-to-evaluate-amp-upgrade-model-versions-in-the-azure-openai/ba-p/4218880). We recommend using this guide as part of the [model upgrade and retirement process](./concepts/model-retirements).
...
The [file search](./how-to/file-search) tool for Assistants now has additional charges for usage. See the [pricing page](https://azure.microsoft.com/pricing/details/cognitive-services/openai-service/) for more information.
...
The model is currently available for both [standard and global standard deployment](./how-to/deployment-types) in the East US region.

For information on model quota, consult the [quota and limits page](./quotas-limits) and for the latest info on model availability refer to the [models page](./concepts/models).
...
Refer to our [Default safety policy documentation](./concepts/default-safety-policies) for more information.
...
Refer to our [data plane inference reference documentation](./reference) for more information.

### Expansion of regions available for global standard deployments of gpt-4o

 GPT-4o is now available for [global standard deployments](./how-to/deployment-types) in:
...
For information on global standard quota, consult the [quota and limits page](./quotas-limits).
...
Refer to our [model retirement guide](./concepts/model-retirements) for the latest information on model deprecation and retirement.
...
For the latest information on model availability, see the [models page](./concepts/models).
...
* [Max completion and max prompt token support](./concepts/assistants) for managing token usage.
...
* Experiment with [Logic Apps and Function Calling using Azure OpenAI Studio](./how-to/assistants-logic-apps). Import your REST APIs implemented in Logic Apps as functions and the studio invokes the function (as a Logic Apps workflow) automatically based on the user prompt.
...
For information on model regional availability, see the [models page](./concepts/models).
...
- For more information, see the [API version lifecycle](./api-version-lifecycle).

### GPT-4 Turbo model general availability (GA)

import Gpt4Turbo from '/snippets/ai-foundry/openai/includes/gpt-4-turbo.mdx'
...
[Use Risks & Safety monitoring](./how-to/risks-safety-monitor)
...
Features that are in preview such as Assistants, text to speech (TTS), and some of the "on your data" datasources, require a preview API version. For more information, check out our [API version lifecycle guide](./api-version-lifecycle).
...
Try out Whisper by following a [quickstart](./whisper-quickstart).
...
Try out DALL-E 3 by following a [quickstart](./dall-e-quickstart).
...
We have added a page to track [model deprecations and retirements](./concepts/model-retirements) in Azure OpenAI. This page provides information about the models that are currently available, deprecated, and retired.
...
If you're currently using a preview API version to take advantage of the latest features, we recommend consulting the [API version lifecycle](./api-version-lifecycle) article to track how long your current API version will be supported.
...
For more information on the upgrade process refer to the [models page](./concepts/models).
...
For information on model regional availability and upgrades refer to the [models page](./concepts/models).
...
- [Concepts](./concepts/assistants)
- [In-depth Python how-to](./how-to/assistant)
...
Azure OpenAI now supports text to speech APIs with OpenAI's voices. Get AI-generated speech from the text you provide. To learn more, see the [overview guide](../../ai-services/speech-service/openai-voices) and try the [quickstart](./text-to-speech-quickstart).
...
- [Fine-tuning & function calling](./how-to/fine-tuning-functions)
...
- [Azure OpenAI On Your Data](./concepts/use-your-data) is now generally available.
...
- New article for using [Azure OpenAI On Your Data configuration](./how-to/on-your-data-configuration) by protecting data with virtual networks and private endpoints.
...
- Explore the capabilities of GPT-4 Turbo with Vision in a no-code experience using the [Azure OpenAI Playground](https://oai.azure.com/). Learn more in the [Quickstart guide](./gpt-v-quickstart).
...
Both models are the latest release from OpenAI with improved instruction following, [JSON mode](./how-to/json-mode), [reproducible output](./how-to/reproducible-output), and parallel function calling.
...
For information on model regional availability consult the [models page](./concepts/models).

The models have their own unique per region [quota allocations](./quotas-limits).
...
- **Expanded customer configurability**: All Azure OpenAI customers can now configure all severity levels (low, medium, high) for the categories hate, violence, sexual and self-harm, including filtering only high severity content. [Configure content filters](./how-to/content-filters)
...
- **New RAI models**
    ...
    [Configure content filters](./how-to/content-filters)

- **Blocklists**: Customers can now quickly customize content filter behavior for prompts and completions further by creating a custom blocklist in their filters. The custom blocklist allows the filter to take action on a customized list of patterns, such as specific terms or regex patterns. In addition to custom blocklists, we provide a Microsoft profanity blocklist (English). [Use blocklists](./how-to/use-blocklists)
## October 2023

### New fine-tuning models (preview)

- `gpt-35-turbo-0613` is [now available for fine-tuning](./how-to/fine-tuning).

- `babbage-002` and `davinci-002` are [now available for fine-tuning](./how-to/fine-tuning). These models replace the legacy ada, babbage, curie, and davinci base models that were previously available for fine-tuning.
...
- Fine-tuned models have different [quota limits](quotas-limits) than regular models.

- [Tutorial: fine-tuning GPT-3.5-Turbo](./tutorials/fine-tune)
...
### GPT-4
GPT-4 and GPT-4-32k are now available to all Azure OpenAI customers. Customers no longer need to apply for the waitlist to use GPT-4 and GPT-4-32k (the Limited Access registration requirements continue to apply for all Azure OpenAI models). Availability might vary by region. Check the [models page](concepts/models), for the latest information on model availability in each region.

### GPT-3.5 Turbo Instruct

Azure OpenAI now supports the GPT-3.5 Turbo Instruct model. This model has performance comparable to `text-davinci-003` and is available to use with the Completions API. Check the [models page](concepts/models), for the latest information on model availability in each region.

### Whisper public preview

Azure OpenAI now supports speech to text APIs powered by OpenAI's Whisper model. Get AI-generated text based on the speech audio you provide. To learn more, check out the [quickstart](./whisper-quickstart).

<Note>
Azure AI Speech also supports OpenAI's Whisper model via the batch transcription API. To learn more, check out the [Create a batch transcription](../../ai-services/speech-service/batch-transcription-create.md#use-a-whisper-model) guide. Check out [What is the Whisper model?](../../ai-services/speech-service/whisper-overview) to learn more about when to use Azure AI Speech vs. Azure OpenAI.
</Note>

### New Regions

- Azure OpenAI is now also available in the Sweden Central, and Switzerland North regions. Check the [models page](concepts/models), for the latest information on model availability in each region.

### Regional quota limits increases

- Increases to the max default quota limits for certain models and regions. Migrating workloads to [these models and regions](./quotas-limits) will allow you to take advantage of higher Tokens per minute (TPM).  
...
- [Azure OpenAI now supports function calling](./how-to/function-calling) to enable you to work with functions in the chat completions API.
...
- Azure OpenAI is now also available in the Canada East, East US 2, Japan East, and North Central US regions. Check the [models page](concepts/models), for the latest information on model availability in each region.  

## June 2023

### Use Azure OpenAI on your own data (preview)

- [Azure OpenAI On Your Data](./concepts/use-your-data) is now available in preview, enabling you to chat with OpenAI models such as GPT-35-Turbo and GPT-4 and receive responses based on your data. 
...
- Azure OpenAI is now available in the UK South region. Check the [models page](concepts/models), for the latest information on model availability in each region.  

### Content filtering & annotations (Preview)

- How to [configure content filters](how-to/content-filters) with Azure OpenAI.
- [Enable annotations](concepts/content-filter) to view content filtering category and severity information as part of your GPT based Completion and Chat Completion calls.

### Quota

- Quota provides the flexibility to actively [manage the allocation of rate limits across the deployments](how-to/quota) within your subscription.
...
- Azure OpenAI is now available in the France Central region. Check the [models page](concepts/models), for the latest information on model availability in each region.  

## April 2023

- **DALL-E 2 public preview**. Azure OpenAI now supports image generation APIs powered by OpenAI's DALL-E 2 model. Get AI-generated images based on the descriptive text you provide. To learn more, check out the [quickstart](./dall-e-quickstart).
...
- **New Chat Completion API for GPT-35-Turbo and GPT-4 models released in preview on 3/21**. To learn more, check out the [updated quickstarts](./quickstart) and [how-to article](./how-to/chatgpt).

- **GPT-35-Turbo preview**. To learn more, check out the [how-to article](./how-to/chatgpt).
...
- [Monitoring an Azure OpenAI](./how-to/monitor-openai)
- [Plan and manage costs for Azure OpenAI](./how-to/manage-costs)
...
**Content filtering is temporarily off** by default. Azure content moderation works differently than Azure OpenAI. Azure OpenAI runs content filters during the generation call to detect harmful or abusive content and filters them from the response. [Learn More​](./concepts/content-filter)
