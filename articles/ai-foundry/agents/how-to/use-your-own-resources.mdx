...
* If configuring a [standard setup](#choose-basic-or-standard-agent-setup), the same individual must also have permissions to assign roles to required resources (Cosmos DB, Search, Storage). For more information about RBAC in Azure AI Foundry, see [RBAC in Azure AI Foundry](../../../ai-foundry/concepts/rbac-azure-ai-foundry).
...
* Register providers. The following providers must be registered:
    ...
    * To use the [Grounding with Bing Search tool](./tools/bing-grounding): `Microsoft.Bing`
...
* Learn about the different [tools](tools\overview) agents can use. 
