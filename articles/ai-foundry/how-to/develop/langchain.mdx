...
* A model deployment supporting the [Model Inference API](https://aka.ms/azureai/modelinference) deployed. In this example, we use a `Mistral-Large-2411` deployment in the [Foundry Models](../../../ai-foundry/model-inference/overview).
...
In the following example, we create two model clients. One is a producer and another one is a verifier. To make the distinction clear, we're using a multi-model endpoint like the [Foundry Models API](../../model-inference/overview) and hence we're passing the parameter `model` to use a `Mistral-Large` and a `Mistral-Small` model, quoting the fact that **producing content is more complex than verifying it**.
...
2. Using the Azure AI Foundry SDK and the project connection string (**[!INCLUDE [hub-project-name](../../includes/hub-project-name)]s only**).
...
3. Identify the trace you have created. It may take a couple of seconds for the trace to show.
    ...
    <Frame>
        ![](/images/ai-foundry/media/how-to/develop-langchain/langchain-portal-tracing-example.png)
    </Frame>

Learn more about [how to visualize and manage traces](visualize-traces).

## Next steps

* [Develop applications with LlamaIndex](llama-index)
* [Visualize and manage traces in Azure AI Foundry](visualize-traces)
* [Use Azure AI Foundry Models](../../model-inference/overview)
* [Reference: Model Inference API](../../../ai-foundry/model-inference/reference/reference-model-inference-api)
