...
1. Select **+ More tools** > **Index Lookup** to add the Index Lookup tool to your flow.
    ...
    <Frame>
        ![](/images/ai-foundry/media/prompt-flow/configure-index-lookup-tool.png)
    </Frame>

1. Enter values for the Index Lookup tool [input parameters](#inputs). The large language model [(LLM) tool](llm-tool) can generate the vector input.
