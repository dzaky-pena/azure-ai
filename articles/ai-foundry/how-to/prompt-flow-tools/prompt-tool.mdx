...
Prepare a prompt. The [LLM tool](llm-tool) and Prompt tool both support [Jinja](https://jinja.palletsprojects.com/en/stable/) templates.
...
For more information and best practices, see [Prompt engineering techniques](../../openai/concepts/advanced-prompt-engineering).
...
1. Select **+ Prompt** to add the Prompt tool to your flow.
    ...
    <Frame>
        ![](/images/ai-foundry/media/prompt-flow/prompt-tool.png)
    </Frame>
...
1. Add more tools (such as the [LLM tool](llm-tool)) to your flow, as needed. Or select **Run** to run the flow.
