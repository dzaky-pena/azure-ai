...
- Read the [When to use Azure OpenAI fine-tuning guide](../concepts/fine-tuning-considerations).
- An Azure subscription. <a href="https://azure.microsoft.com/free/cognitive-services" target="_blank">Create one for free</a>.
- An Azure OpenAI resource that's located in a region that supports fine-tuning of the Azure OpenAI model. Check the [Model summary table and region availability](../concepts/models.md#fine-tuning-models) for the list of available models by region and supported functionality. For more information, see [Create a resource and deploy a model with Azure OpenAI](../how-to/create-resource).
- Fine-tuning access requires **Cognitive Services OpenAI Contributor**.
- If you do not already have access to view quota, and deploy models in Azure AI Foundry portal you will require [additional permissions](../how-to/role-based-access-control).  
...
The training and validation data you use **must** be formatted as a JSON Lines (JSONL) document and must be formatted in the conversational format that is used by the [Chat completions](../how-to/chatgpt) API.
...
1. In Azure AI Foundry portal, browse to the **Tools > Fine-tuning** pane, and select **Fine-tune model**.
   ...
   <Frame>
      ![](/images/ai-foundry/openai/media/fine-tuning/studio-create-custom-model.png)
   </Frame>

1. Select a model to fine-tune, and then select **Next** to continue.

   <Frame>
      ![](/images/ai-foundry/openai/media/fine-tuning/studio-model-selection.png)
   </Frame>

You should now see the **Create a fine-tuned model** dialog.

<Frame>
   ![](/images/ai-foundry/openai/media/fine-tuning/studio-create-fine-tuned-model.png)
</Frame>
...
- **Direct Preference Optimization (Preview)** ([DPO](../how-to/fine-tuning-direct-preference-optimization)): supported by GPT-4o.
- **Reinforcement Fine Tuning (Preview)** ([RFT](../how-to/reinforcement-fine-tuning)): supported by reasoning models, like o4-mini.
...
<Frame>
   ![](/images/ai-foundry/openai/media/fine-tuning/studio-training-data.png)
</Frame>
...
<Frame>
   ![](/images/ai-foundry/openai/media/fine-tuning/studio-validation-data.png)
</Frame>
...
<Frame>
   ![](/images/ai-foundry/openai/media/fine-tuning/studio-create-hyperparams.png)
</Frame>
...
To save time, you can optionally enable auto-deployment for your resulting model. If training completes successfully, the model will be deployed using the selected [deployment type](../how-to/deployment-types). The deployment will be named based on the unique name generated for your custom model and the optional **suffix** you may have provided [earlier](#make-your-model-identifiable-optional).

<Frame>
   ![](/images/ai-foundry/openai/media/fine-tuning/studio-create-auto-deploy.png)
</Frame>

<Note>
Only Global Standard and Developer deployments are currently supported for auto-deployment. Neither of these options provide [data residency](https://aka.ms/data-residency). Consult the [deployment type](../how-to/deployment-types) documentation for more details.
</Note>
...
<Note>
For chat models, the [system message that you use to guide your fine-tuned model](../concepts/system-message) (whether it's deployed or available for testing in the playground) must be the same as the system message you used for training. If you use a different system message, the model might not perform as expected.
</Note>
...
<Frame>
   ![](/images/ai-foundry/openai/media/fine-tuning/studio-continuous.png)
</Frame>
...
import FineTuningDeletion from '/snippets/ai-foundry/openai/includes/fine-tune.mdx'
